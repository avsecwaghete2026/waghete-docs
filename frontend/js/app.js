...
const els = {
  ...
  adminUploadView: document.getElementById("view-admin-upload"),
  adminUsersView: document.getElementById("view-admin-users"),
};
...
async function bootstrap() {
  ...
  els.toggle.addEventListener("click", () => {
    ...
    try {
      localStorage.setItem("waghete:sidebar", collapsed ? "1" : "0");
    } catch (_) {}
  });
  ...
  // Show the Upload and Accounts tabs only for admins. Use style.display
  // so this bypasses the CSS cascade entirely.
  const isAdmin = profile.role === "admin";
  els.uploadTab.style.display = isAdmin ? "" : "none";
  els.usersTab.style.display = isAdmin ? "" : "none";
  if (isAdmin) {
    await initAdmin();
  }
...
function applySidebarState() {
  let collapsed = false;
  try {
    collapsed = localStorage.getItem("waghete:sidebar") === "1";
  } catch (_) {}
  els.sidebar.classList.toggle("collapsed", collapsed);
  els.toggle.setAttribute("aria-expanded", String(!collapsed));
}
...
function switchTab(view) {
  ...
  els.adminUploadView.classList.toggle("active", view === "admin-upload");
  els.adminUsersView.classList.toggle("active", view === "admin-users");
}

function renderAvatar(img, email) {
  // Generate a consistent color avatar from the email hash.
  const hue = [...email].reduce(
